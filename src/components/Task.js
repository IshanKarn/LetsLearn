...
const Task = ({ task, phaseIndex, weekIndex, dayIndex, taskIndex, updateRoadmap }) => {
  ...
  const addNote = async (category) => {
    if (newNote[category] && newNote[category].trim()) {
      try {
        setError(null);
        const updatedNotes = { ...task.notes, [category]: [...task.notes[category], { id: null, content: newNote[category] }] };

...
const Week = ({ week, phaseIndex, weekIndex, updateRoadmap }) => {
  return (
        ...
        <>
          ...
          <Disclosure.Panel className="px-4 pb-2 pt-4 text-sm text-gray-500">
            {week.days.map((day, dayIndex) => (
              <Day key={dayIndex} day={day} phaseIndex={phaseIndex} weekIndex={weekIndex} dayIndex={dayIndex} updateRoadmap={updateRoadmap} />
            ))}
          </Disclosure.Panel>
        </>
      )}
